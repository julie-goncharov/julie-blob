...
this.displayAuthor = function(string) {
    ...
    if (string.includes('[') || string.includes('{')) {
        arrayString = string.replace(/[\[\{\]\}"]/g, "").split(new RegExp(",| and "));
    } else {
        arrayString = string.split(new RegExp("[\\s]+and[\\s]+"));
    }

    // Adjust each author name to "First Last" format
    arrayString = arrayString.map(name => {
        var parts = name.trim().split(' ');
        if (parts.length > 1 && parts[0].endsWith(',')) {  // It's in the "Last, First" format
